...
const Greetings = () => {
  ...
  const getGreetings = async () => {
    try {
      const response = await axios.get('http://192.168.1.40:5000/api/greet/greet');
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await axios.patch(
        `http://192.168.1.40:5000/api/greet/greet/${editedGreet.id}`,
  ...
  //   try {
  //     await axios.delete(`http://192.168.1.40:5000/api/greet/greet/${itemIdToDelete}`);
  ...
  const createGreet = async () => {
    ...
    try {
      if (!dataAdded) {
        const response = await axios.post('http://192.168.1.40:5000/api/greet/greet', {
          greet: trimmedGreet,
  ...
  const updateGreet = async (greetId) => {
    ...
    try {
      const response = await axios.patch(`http://192.168.1.40:5000/api/greet/greet/${greetId}`, updateGreetData);
