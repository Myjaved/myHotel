...
const [errorMessage, setErrorMessage] = useState("");
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      await axios.patch(
        `http://192.168.1.40:5000/api/section/${sectionToEdit._id}`,
        {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      await axios.delete(
        `http://192.168.1.40:5000/api/section/${sectionToDelete._id}`
      );
      ...
      const sectionsResponse = await axios.get(
        "http://192.168.1.40:5000/api/section"
      );
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.40:5000/api/section/create",
        formData,
        {
          headers: {
