...
const Billing = ({ tableId, acPercentage }) => {
  ...
  //       (orderItem) => !selectedMenuIds.includes(orderItem.name)
  //     );
  //     console.log(updatedOrder)
  //     setCurrentOrder(updatedOrder);


  //     // Clear selected menu IDs
  //     setSelectedMenuIds([]);
    ...
    const fetchTastes = async () => {
      try {
        const response = await axios.get(
          "http://192.168.1.40:5000/api/taste/tastes"
        );
  ...
  const fetchWaitersList = async () => {
    try {
      const response = await axios.get("http://192.168.1.40:5000/api/waiter");
  ...
  const fetchLastAllOrders = async () => {
    try {
      const ordersResponse = await axios.get(
        "http://192.168.1.40:5000/api/order/latest-orders"
      );
      ...
      const tableNamesPromises = orders.map(async (order) => {
        const tableResponse = await axios.get(
          `http://192.168.1.40:5000/api/table/tables/${order.tableId}`
        );
  ...
  const saveBill = async () => {
    try {
      ...
      const KOTResponse = await axios.post(
        `http://192.168.1.40:5000/api/kot/kotOrder/${tableId}`,
        kotData
      );
...
<script>
  ...
  useEffect(() => {
    ...
    // Fetch categories
    axios
      .get("http://192.168.1.40:5000/api/main")
      .then((response) => {
    ...
    // Fetch products
    axios
      .get("http://192.168.1.40:5000/api/menu/menus/list")
      .then((response) => {
        // console.log(response.data);
  ...
  const handleCategoryClick = (category) => {
    ...
    if (category === null) {
      ...
      // Fetch menus based on the selected category
      axios
        .get(`http://192.168.1.40:5000/api/menu/menulist/${category._id}`)
        .then((response) => {
          setMenus(response.data);
        })
        .catch((error) => {
    ...
    const fetchHotelInfo = async () => {
      try {
        ...
        const allHotelsResponse = await axios.get(
          "http://192.168.1.40:5000/api/hotel/get-all"
        );
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(
            `http://192.168.1.40:5000/api/hotel/get/${defaultHotelId}`
          );
    ...
    <div className=" font-sans mt-2">
      ...
      <div className="container mx-auto bg-white ">
        <div className="flex lg:flex-row shadow-lg ">
          <div className=" w-80 lg:w-2/5 bg-gray-200 -mt-3 md:w-96">
            {/* Header */}
            <div className="font-bold text-sm px-3 flex mt-2">
              Last Bills
            ...
            {tableInfo ? (
                ...
                <div className="p-1 custom-scrollbars overflow-y-auto lg:h-72 md:h-40  max-h-[calc(96vh-1rem)] lg:text-sm md:text-sm text-xs">
                  {currentOrder.map((orderItem) => (
                    <div key={orderItem._id} className="flex items-center mb-2">
                        ...
                        <div className="float-right flex justify-between  md:ml-1 mt-1">
                         ...
                         font-bold p-1 lg:w-4 lg:text-md md:w-4 sm:w-4"
                          ...
                          <input
                            ...
                            className="font-semibold lg:w-10  w-7 justify-center flex text-center rounded-md align-center  ml-1 md:text-xs pl-0"
                            min={1}
                          />
                          <span
                      ...
                      <div className="font-semibold  lg:text-base md:text-md text-xs mt-1  text-right lg:-ml-3  ml-1 lg:mt-0  md:mt-0 sm:mt-0  sm:text-xs sm:w-20 ">
              ...
              <div className="font-bold lg:text-xl text-sm md:text-xl lg:px-28 px-20 mt-2 flex  sm:text-lg">
          ...
          <div className=" w-72 lg:w-3/5  xl: shadow-lg md:w-96 ">
            ...
            <div
              className="cursor-pointer grid grid-cols-2 bg-white  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1
               lg:px-3 md:px-2 px-2 mt-3 custom-sidescrollbars overflow-scroll lg:max-h-[calc(82vh-1rem)]
              ...
              {(menus.menus || menus)
                ...
                .map((product, index) => (
                  <div
                    ...
                    h-24 justify-between text-sm sm:h-24 lg:h-32 md:h-28 "
                    ...
                    <div>
                      {product.uniqueId && (
                        <div className="lg:-mt-3">
                          <span className="text-orange-500 md:text-xs  text-lg font-semibold lg:text-sm rounded-md overflow-hidden whitespace-nowrap">{product.uniqueId}</span>
                          <span className=" float-right text-green-700 md:text-xs  text-lg font-bold lg:text-sm rounded-md overflow-hidden whitespace-nowrap" style={{ fontSize: '12px' }}>
                            ₹{product.price}
                          </span>
                        </div>
                      )}
                      <div className="justify-center flex">
                        <img
                          src={product.imageUrl ? `http://192.168.1.40:5000/${product.imageUrl}` : `/tray.png`}
                          className={`object-cover rounded-md ${product.imageUrl
                            ? 'lg:w-24 lg:h-16 md:w-10 md:h-10 w-8 h-8 lg:mt-1 -mt-4 md:-mt-1'
                            : 'lg:w-12 lg:h-10 md:w-7 md:h-7 w-8 h-8 lg:mt-3 -mt-3 md:-mt-1'
                            }`}
                          alt=""
                        />
                      </div>
                      <div className="font-bold text-gray-800 md:mt-1 sm:mt-1 lg:mt-1 lg:flex justify-between">
                        <span className="md:text-xs sm:text-xs lg:mb-1" style={{ fontSize: '13px', maxHeight: '2em', display: 'block' }}>{product.name}</span>
                        <span>{product.stockQty ? (
