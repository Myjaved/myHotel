    ...
    useEffect(() => {
        const fetchACPercentage = async () => {
            try {
                ...
                const tableResponse = await axios.get(`http://192.168.1.40:5000/api/table/tables/${tableId}`);
                const table = tableResponse.data;

                // Fetch the section information using the section ID
                const sectionResponse = await axios.get(`http://192.168.1.40:5000/api/section/${table.section._id}`);
