    ...
    const fetchData = async () => {
      try {
        ...
        const tablesResponse = await axios.get(
          "http://192.168.1.40:5000/api/table/tables"
        );
        ...
        <div>
          <ul className=" grid grid-cols-2 bg-white  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 px-5">
            {sections.map((section) => (
              <li
                key={section._id}
                className="mb-2 md:mb-0 bg-white  rounded-2xl px-1 shadow-md hover:bg-slate-100"
              >
                <div className="flex items-center lg:-mt-4 custom-scrollbars overflow-x-auto">
