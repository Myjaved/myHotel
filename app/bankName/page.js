...
const BankNameList = () => {
  ...
  useEffect(() => {
    const fetchBankNames = async () => {
      try {
        const response = await axios.get('http://192.168.1.40:5000/api/bankName/bankNames');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.patch(`http://192.168.1.40:5000/api/bankName/bankNames/${selectedBankName._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`http://192.168.1.40:5000/api/bankName/bankNames/${selectedBankName._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://192.168.1.40:5000/api/bankName/bankNames', { bankName: newBankName });
