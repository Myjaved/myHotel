...
const CreateHotelForm = () => {
  ...
  const fetchHotels = async () => {
    try {
      const response = await axios.get('http://192.168.1.40:5000/api/hotel/get-all');
  ...
  const handleEdit = async (hotelId) => {
    try {
      ...
      const response = await axios.get(`http://192.168.1.40:5000/api/hotel/get/${hotelId}`);
...
const handleEditInputChange = (e) => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`http://192.168.1.40:5000/api/hotel/edit/${editFormData._id}`, formDataForUpload);
  ...
  //   try {
  //     await axios.delete(`http://192.168.1.40:5000/api/hotel/delete/${hotelId}`);
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://192.168.1.40:5000/api/hotel/delete/${deleteHotelId}`);
      console.log('Hotel deleted successfully.');
      fetchHotels(); // Refresh the list after deletion
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://192.168.1.40:5000/api/hotel/create', formDataForUpload);
              ...
              <tbody>
                {hotels.map((hotel) => (
                  <tr
                    ...
                    {hotel.hotelLogo && <img src={`http://192.168.1.40:5000/${hotel.hotelLogo}`} alt="Hotel Logo" className="w-10 h-10" />}
                    </td>
