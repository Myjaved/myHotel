...
const NewItemModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      try {
        const itemsResponse = await axios.get("http://192.168.1.40:5000/api/item/items");
  ...
  useEffect(() => {
    ...
    const fetchUnits = async () => {
      try {
        const unitsResponse = await axios.get("http://192.168.1.40:5000/api/unit/units");
    ...
    if (name === "unit") {
      ...
      try {
        const unitDetailsResponse = await axios.get(
          `http://192.168.1.40:5000/api/unit/units/${value}`
        );
    ...
    try {
      ...
      if (itemToEdit) {
        ...
        response = await axios.put(
          `http://192.168.1.40:5000/api/item/items/${itemToEdit._id}`,
          formData
        );
        ...
        response = await axios.post("http://192.168.1.40:5000/api/item/items", {
          ...formData,
          unit: formData.unit || "",
...
const NewUnitModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        const response = await axios.get('http://192.168.1.40:5000/api/unit/units');
  ...
  const handleEditSubmit = async (e, updatedUnit) => {
    ...
    try {
      ...
      await axios.patch(`http://192.168.1.40:5000/api/unit/units/${selectedUnit._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`http://192.168.1.40:5000/api/unit/units/${selectedUnit._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('http://192.168.1.40:5000/api/unit/units', { unit: newUnit });
...
const NewGstModal = ({ isOpen, onClose }) => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete(`http://192.168.1.40:5000/api/gst/gst/${deletingItemId}`);
  ...
  const fetchGSTList = async () => {
    try {
      const response = await axios.get('http://192.168.1.40:5000/api/gst/list');
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await axios.post('http://192.168.1.40:5000/api/gst/create', { gstPercentage });
      fetchGSTList();
      setGSTPercentage('');
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.40:5000/api/supplier/suppliers",
        formData
      );
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(
        `http://192.168.1.40:5000/api/supplier/suppliers/${editedSupplier._id}`,
        editedSupplier
      );
...
const PurchaseForm = () => {
  ...
  useEffect(() => {
    const fetchStockQty = async () => {
      try {
        const response = await axios.get(
          `http://192.168.1.40:5000/api/purchase/purchase/stockQty?itemName=${formData.itemName}`
        );
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          "http://192.168.1.40:5000/api/item/items"
        );
  ...
  useEffect(() => {
    const fetchUnitList = async () => {
      try {
        const response = await axios.get(
          "http://192.168.1.40:5000/api/unit/units"
        );
        setUnits(response.data);
        console.log(response.data);
      } catch (error) {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.40:5000/api/purchase/purchase/savebill",
        data
      );
      ...
      if (parseFloat(data.total) > 0) {
        ...
        await axios.post("http://192.168.1.40:5000/api/supplier/supplier/debit", {
          vendorName,
          amount: parseFloat(data.paidAmount),
        });
      } else {
        // If there is no paid amount, update the supplier's credit balance
        await axios.post("http://192.168.1.40:5000/api/supplier/supplier/credit", {
          vendorName,
