...
const SupplierPayment = () => {
  ...
  useEffect(() => {
    axios.get('http://192.168.1.40:5000/api/supplier/suppliers')
  ...
  useEffect(() => {
    if (selectedSupplier) {
      axios.get(`http://192.168.1.40:5000/api/supplier/suppliers/${selectedSupplier}`)
        .then(response => {
  ...
  const handleDebitSubmission = (e) => {
    ...
    if (debitAmount <= creditBalance && !loading) {
      ...
      axios.put(`http://192.168.1.40:5000/api/supplier/updateBalance/${selectedSupplier}`, {
        debit: debitAmount,
      })
        .then(response => {
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      await axios.delete(`http://192.168.1.40:5000/api/supplier/suppliers/${selectedSupplier}/dateWiseRecords/${recordIdToDelete}`);
