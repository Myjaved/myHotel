...
const StockOutwardTable = () => {
  ...
  const fetchStockOutwardList = async (start, end) => {
    try {
      const response = await axios.get('http://192.168.1.40:5000/api/stockOut/stockOut', {
        params: {
          startDate: start,
          endDate: end,
        },
  ...
  const handleExportItemsToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(itemList.map(({ itemName, stockQty }) => ({ 'Item Name': itemName, 'Remaining Stock': stockQty })));
    const wb = XLSX.utils.book_new();
