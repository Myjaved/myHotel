...
const Waiter = () => {
  ...
  const handleAddWaiter = async () => {
    ...
    if (waiterData.contactNumber.length !== 10 || isNaN(waiterData.contactNumber)) {
      ...
      const response = await axios.post('http://192.168.1.40:5000/api/waiter', waiterData);
  ...
  //   try {
  //     const response = await axios.put(`http://192.168.1.40:5000/api/waiter/${waiterId}`, waiterData);
  ...
  const confirmDeleteWaiter = async () => {
    try {
      await axios.delete(`http://192.168.1.40:5000/api/waiter/${deletingWaiterId}`);
