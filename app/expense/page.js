...
const ExpenseList = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const response = await axios.get('http://192.168.1.40:5000/api/expense/expenses');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      await axios.patch(`http://192.168.1.40:5000/api/expense/expenses/${selectedExpense._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`http://192.168.1.40:5000/api/expense/expenses/${selectedExpense._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://192.168.1.40:5000/api/expense/expenses', { expense: newExpense });
