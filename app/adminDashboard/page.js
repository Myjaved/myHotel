...
//       try {
//         const response = await axios.get('http://192.168.1.40:5000/api/item/low-stock');
...
//       try {
//         const response = await axios.get('http://192.168.1.40:5000/api/supplier/openingBalances');
//         const data = response.data;
...
//       try {
//         const response = await axios.get('http://192.168.1.40:5000/api/order/itemsWithSameNameAndMenu');
//         const data = response.data;
...
const AdminDashboard = () => {
  ...
  const [lowStockItems, setLowStockItems] = useState([]);
  const [openingBalances, setOpeningBalances] = useState([]);
  const [itemsWithSameNameAndMenu, setItemsWithSameNameAndMenu] = useState([]);

  // Fetch data on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch data for low stock items
        const lowStockResponse = await axios.get('http://192.168.1.40:5000/api/item/low-stock');
        setLowStockItems(lowStockResponse.data);

        // Fetch data for opening balances
        const openingBalancesResponse = await axios.get('http://192.168.1.40:5000/api/supplier/openingBalances');
        setOpeningBalances(openingBalancesResponse.data);

        // Fetch data for items with the same name and menu
        const itemsWithSameNameAndMenuResponse = await axios.get('http://192.168.1.40:5000/api/order/itemsWithSameNameAndMenu');
        setItemsWithSameNameAndMenu(itemsWithSameNameAndMenuResponse.data);
      } catch (error) {
